...
VkInstanceCreateInfo* tp_create_vk_inst_creat_inf(VkApplicationInfo* p_vk_app_inf)
{
    ...
    uint32_t glfw_ext_cntr               = 0;

    /* TODO: Find out why I had to move this to the user programm, so that it would open a window */
    /* Instead of hanging at #2  0x00007ffff7a64ba5 in XSetErrorHandler() from /usr/lib/libX11.so.6 */
    /** if(!glfwInit()) {
        printf("Initializing GLFW failed. Needed to get required instance extensions\n");
        exit(-3);
    } **/
    /* Probably the lifecycle of something ends that is still needed later on beause we are in a local function */

    const char** prelim_required_exts    = glfwGetRequiredInstanceExtensions(&glfw_ext_cntr);
    char** required_exts                 = malloc(glfw_ext_cntr * sizeof(char*));
    ...
    p_vk_inst_creat_inf->enabledExtensionCount   = glfw_ext_cntr;
...
Tp_Vk_Inst_Fam_Plmsone* tp_init_prelim_stg_one_strct_vk_inst_fam(const char* app_nm, uint32_t maj, uint32_t min, uint32_t pat)
{
    VkApplicationInfo* p_vk_app_inf = tp_create_vk_app_inf(app_nm, maj, min, pat);
    VkInstanceCreateInfo* p_vk_inst_create_inf = tp_create_vk_inst_creat_inf(p_vk_app_inf);
...
Tp_Init_Vk_Stg_One* tp_super_init_vulkan_stg_one(const char* app_nm, uint32_t maj, uint32_t min, uint32_t pat)
{
    ...
    Tp_Vk_Inst_Fam_Plmsone* p_tp_vk_inst_fam       = tp_init_prelim_stg_one_strct_vk_inst_fam(p_tp_strct_stg_one->app_nm, p_tp_strct_stg_one->maj, p_tp_strct_stg_one->min, p_tp_strct_stg_one->pat);
...
VkSurfaceKHR* tp_glfw_create_win_surface_wrapper(Tp_Init_Vk_Stg_One* tp_init_vk_stg_one_data, GLFWwindow* glfw_win)
{
...
Tp_Sur_Plmssix* tp_init_sur(Tp_Init_Vk_Stg_One* p_tp_init_vk_stg_one_data, const uint32_t width, const uint32_t height, const char* title)
{
    ...
    **/
    ...
    p_prelim_stg_six_strct_sur_data->p_glfw_window          = win;
...
Tp_Init_Vk_Stg_Six* tp_super_init_vulkan_stg_six(Tp_Init_Vk_Stg_Five* p_tp_stg_five_data, uint32_t width, uint32_t height, const char* title)
{
    ...
    Tp_Sur_Plmssix*           p_prelim_stg_six_strct_vk_sur_khr_data    = tp_init_sur(p_tp_stg_one_data, width, height, title);
